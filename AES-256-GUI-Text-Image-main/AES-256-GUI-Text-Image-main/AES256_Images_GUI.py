import tkinter as tk #Library to handle with image
from tkinter import filedialog #Open a dialog to select images
from PIL import Image, ImageTk #To handle with images
from Crypto.Cipher import AES #In Aes algorithm
from Crypto.Hash import SHA256 #Hash
from Crypto.Util.Padding import pad, unpad #Using in oadding
from Crypto.Util import Counter #In ctr
import os
...
def encrypt_image_ecb(filename):
    ...
    value_vector = im.convert("RGB").tobytes()#convert bytes to RGB
    imlength = len(value_vector)#Find the lenght of image 
    aesCipher_ECB = AES.new(hkey, AES.MODE_ECB)#Choise the mode in encryption
    cipher_vector = aesCipher_ECB.encrypt(pad_img(value_vector)[:imlength]) #Encryption
    cipher_img = trans_format_RGB(cipher_vector)#Convert the encryption data to RGB to show the encryption Image
    im2 = Image.new(im.mode, im.size)#Consume the image with the same of mode and size
    im2.putdata(cipher_img)#Put the new encryption data in new image
    im2.save(filename_encrypted_ecb + "." + FORMAT, FORMAT)#Save the image in new file called encrypted_ecb
